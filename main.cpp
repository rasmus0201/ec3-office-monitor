...
void setup()
{
    ...
    for (JsonObject data_sensor : data["sensors"].as<JsonArray>()) {
        bool inserted = manager->SetSensorId(
            data_sensor["name"].as<const char *>(),
            data_sensor["id"].as<uint32_t>()
        );
        
        if (!inserted) {
            printf("Could not insert into map!\n");
        }
    }
